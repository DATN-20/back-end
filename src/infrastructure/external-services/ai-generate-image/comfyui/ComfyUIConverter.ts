...
export class ComfyUIConverter {
  ...
  async convertToComfyUIPromptText2Img(input_prompts: InputPromts): Promise<any> {
    ...
    workflow['1']['inputs']['ckpt_name'] = input_prompts.style;
    workflow['2']['inputs']['text'] = input_prompts.positivePrompt;
    workflow['3']['inputs']['text'] = input_prompts.negativePrompt;
    workflow['8']['inputs']['width'] = input_prompts.width;
    workflow['8']['inputs']['height'] = input_prompts.height;
    workflow['8']['inputs']['batch_size'] = input_prompts.numberOfImage;
    if (input_prompts.seed != null) {
      workflow['9']['inputs']['seed'] = input_prompts.seed;
    } else {
      const seed = Math.floor(Math.random() * 1000000000) + 1;
      workflow['9']['inputs']['seed'] = seed;
    }
    workflow['9']['inputs']['steps'] = input_prompts.steps;
    workflow['9']['inputs']['cfg'] = input_prompts.cfg;
    workflow['9']['inputs']['sampler_name'] = input_prompts.sampleMethos;
  ...
  async convertToComfyUIPromptImg2Img(input_prompts: InputPromts): Promise<any> {
    ...
    workflow['4']['inputs']['steps'] = input_prompts.steps;
    workflow['4']['inputs']['cfg'] = input_prompts.cfg;
    workflow['4']['inputs']['sampler_name'] = input_prompts.sampleMethos;
    workflow['6']['inputs']['image'] = input_prompts.filename;
    workflow['4']['inputs']['denoise'] = input_prompts.noise;
