...
export const images_album = mysqlTable(
  'images_album',
  {
  ...
  table => {
    return {
      pk: primaryKey({ columns: [table.imageId, table.albumId] }),
    };
  },
);
...
export const images_interaction = mysqlTable(
  'images_interaction',
  {
    userId: int('user_id')
      .notNull()
  ...
  table => {
    return {
      pk: primaryKey({ columns: [table.imageId, table.userId, table.type] }),
    };
  },
);
