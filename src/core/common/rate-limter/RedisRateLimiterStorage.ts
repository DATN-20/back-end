...
export class RedisRateLimiterStorage implements IRateLimiterStorage {
  ...
  async getExpiredAt(user: RateLimiterUser, endpoint: string): Promise<Date> {
    const expired_at = await this.redis.ttl(`${RATE_LIMITER_PREFIX}:${user.userId}:${endpoint}`);
    const now = new Date();
  ...
  async setDefaultBucket(
    ...
    const is_existed = !!(await this.redis.get(
      `${RATE_LIMITER_PREFIX}:${user.userId}:${endpoint}`,
    ));
    ...
    await this.redis.set(`${RATE_LIMITER_PREFIX}:${user.userId}:${endpoint}`, bucket_size);
    await this.redis.expire(`${RATE_LIMITER_PREFIX}:${user.userId}:${endpoint}`, refill_rate);
  }

  async decrement(user: RateLimiterUser, endpoint: string): Promise<number> {
    return this.redis.decr(`${RATE_LIMITER_PREFIX}:${user.userId}:${endpoint}`);
  }

  async increment(user: RateLimiterUser, endpoint: string): Promise<number> {
    return this.redis.incr(`${RATE_LIMITER_PREFIX}:${user.userId}:${endpoint}`);
  }
}
